...
For rapid execution of a subset of unit tests (as during test-driven development), the `-k` argument will disable code coverage and pass directly to pytest to limit the tests that are run.
...
To run functional tests, use the following command from the root, top-level directory.
You will need to [install the complete Nextstrain environment](https://nextstrain.org/docs/getting-started/local-installation) and augur dev dependencies to run these tests.
